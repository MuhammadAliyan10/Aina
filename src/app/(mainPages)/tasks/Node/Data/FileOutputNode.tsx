...
const FileOutputNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`FileOutputNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `FileOutputNode ${id}: File written - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
      setError(null);
  ...
  const handleDelete = () => {
    ...
    log.info(`FileOutputNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `FileOutputNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
        ...
        node.id === id
          ? {
              ...
              data: {
                ...node.data,
                description,
                config: { ...node.data.config, filePath, isEnabled },
              },
            }
          : node
      )
    ...
    log.info(
      `FileOutputNode ${id}: Configuration saved - ${description}, File Path: ${filePath}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Write results to JSON"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Write File</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Write File</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {filePath && (
          <div className="text-xs text-gray-300 flex items-center gap-1">
            <FileOutput size={12} />
            <span className="truncate max-w-[9rem]">{filePath}</span>
          </div>
        )}
      </div>
