...
const DatabaseQueryNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`DatabaseQueryNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `DatabaseQueryNode ${id}: Query executed - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`DatabaseQueryNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `DatabaseQueryNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
        ...
        node.id === id
          ? {
              ...
              data: {
                ...node.data,
                description,
                config: { ...node.data.config, query, isEnabled },
              },
            }
          : node
      )
    ...
    log.info(
      `DatabaseQueryNode ${id}: Configuration saved - ${description}, Query: ${query}`
    );
  };
    ...
    >
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Database Query</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Database Query</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {query && (
          <div className="text-xs text-gray-300 flex items-center gap-1">
            <Database size={12} />
            <span className="truncate max-w-[9rem]">{query}</span>
          </div>
        )}
      </div>
