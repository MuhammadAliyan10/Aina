...
const DataFilterNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`DataFilterNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `DataFilterNode ${id}: Filtered - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
      setError(null);
  ...
  const handleDelete = () => {
    ...
    log.info(`DataFilterNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `DataFilterNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const validateInputs = () => {
    ...
    if (!filterCondition.trim()) {
      setError("Filter condition is required");
      return false;
    }
    setError(null);
    return true;
  ...
  const handleSave = () => {
        ...
        node.id === id
          ? {
              ...
              data: {
                ...node.data,
                description,
                config: { ...node.data.config, filterCondition, isEnabled },
              },
            }
          : node
      )
    ...
    log.info(
      `DataFilterNode ${id}: Configuration saved - ${description}, Condition: ${filterCondition}`
    );
  };
                ...
                <DialogContent className="bg-gray-800 text-white rounded-lg shadow-xl p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      >
                        Filter Condition (JS Expression)
                      </Label>
                      <Input
                        id="filterCondition"
                        type="text"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Filter Data</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {filterCondition && (
          <div className="text-xs text-gray-300 flex items-center gap-1">
            <Search size={12} />
            <span className="truncate max-w-[9rem]">{filterCondition}</span>
          </div>
        )}
      </div>
