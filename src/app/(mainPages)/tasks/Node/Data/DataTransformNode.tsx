...
const DataTransformNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`DataTransformNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `DataTransformNode ${id}: Transformed - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
      setError(null);
  ...
  const handleDelete = () => {
    ...
    log.info(`DataTransformNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `DataTransformNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const validateInputs = () => {
    ...
    if (!transformRule.trim()) {
      setError("Transform rule is required");
      return false;
    }
    setError(null);
    return true;
  ...
  const handleSave = () => {
        ...
        node.id === id
          ? {
              ...
              data: {
                ...node.data,
                description,
                config: { ...node.data.config, transformRule, isEnabled },
              },
            }
          : node
      )
    ...
    log.info(
      `DataTransformNode ${id}: Configuration saved - ${description}, Rule: ${transformRule}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder='e.g., {"name": "data.name"}'
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    {error && (
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Transform Data</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Transform Data</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {transformRule && (
          <div className="text-xs text-gray-300 flex items-center gap-1">
            <Cog size={12} />
            <span className="truncate max-w-[9rem]">{transformRule}</span>
          </div>
        )}
      </div>
