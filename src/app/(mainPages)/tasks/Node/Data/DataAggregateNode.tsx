...
const DataAggregateNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [aggregateType, setAggregateType] = useState(
    data.config?.aggregateType || "sum"
  );
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`DataAggregateNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `DataAggregateNode ${id}: Aggregated - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
      setError(null);
  ...
  const handleDelete = () => {
    ...
    log.info(`DataAggregateNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `DataAggregateNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `DataAggregateNode ${id}: Configuration saved - ${description}, Type: ${aggregateType}, Field: ${field}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Sum sales totals"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
                      <Label htmlFor="aggregateType" className="text-gray-300">
                        Aggregate Type
                      </Label>
                      <Select
                        value={aggregateType}
                        onValueChange={setAggregateType}
                      >
                        <SelectTrigger
                          id="aggregateType"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Aggregate Data</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Aggregate Data</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
