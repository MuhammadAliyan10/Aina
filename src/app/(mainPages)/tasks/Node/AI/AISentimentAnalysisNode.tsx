...
const AISentimentAnalysisNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AISentimentAnalysisNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AISentimentAnalysisNode ${id}: Sentiment analyzed - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`AISentimentAnalysisNode ${id}: Deleted`);
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `AISentimentAnalysisNode ${id}: Configuration saved - ${description}, Text: ${text}`
    );
  };
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., The product is amazing!"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
...
export { AISentimentAnalysisNode };
