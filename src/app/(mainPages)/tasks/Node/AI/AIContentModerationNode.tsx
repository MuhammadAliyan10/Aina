...
const AIContentModerationNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AIContentModerationNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AIContentModerationNode ${id}: Content moderated - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`AIContentModerationNode ${id}: Deleted`);
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `AIContentModerationNode ${id}: Configuration saved - ${description}, Level: ${moderationLevel}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Moderate user comments"
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., This is a user comment"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Content Moderation</span>
          <span
...
export { AIContentModerationNode };
