...
const AIImageGenerationNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [prompt, setPrompt] = useState(data.config?.prompt || "");
  const [imageSize, setImageSize] = useState(
    data.config?.imageSize || "512x512"
  );
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AIImageGenerationNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AIImageGenerationNode ${id}: Image generated - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`AIImageGenerationNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `AIImageGenerationNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `AIImageGenerationNode ${id}: Configuration saved - ${description}, Prompt: ${prompt}`
    );
  };
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Generate product image"
                    ...
                    <div>
                      <Label htmlFor="imageSize" className="text-gray-300">
                        Image Size
                      </Label>
