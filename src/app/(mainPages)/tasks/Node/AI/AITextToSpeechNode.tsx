...
const AITextToSpeechNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [text, setText] = useState(data.config?.text || "");
  const [voice, setVoice] = useState(data.config?.voice || "default");
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AITextToSpeechNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AITextToSpeechNode ${id}: Audio generated - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`AITextToSpeechNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `AITextToSpeechNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `AITextToSpeechNode ${id}: Configuration saved - ${description}, Voice: ${voice}`
    );
  };
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Convert text to audio narration"
