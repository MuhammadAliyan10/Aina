...
const AIContextualSearchNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [query, setQuery] = useState(data.config?.query || "");
  const [dataset, setDataset] = useState(data.config?.dataset || "default");
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AIContextualSearchNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AIContextualSearchNode ${id}: Search completed - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`AIContextualSearchNode ${id}: Deleted`);
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `AIContextualSearchNode ${id}: Configuration saved - ${description}, Dataset: ${dataset}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Search company knowledge base"
                    ...
                    <div>
                      <Label htmlFor="dataset" className="text-gray-300">
                        Dataset
                      </Label>
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Contextual Search</span>
          <span
