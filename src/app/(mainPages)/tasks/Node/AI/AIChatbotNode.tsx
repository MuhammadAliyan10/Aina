...
const AIChatbotNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [userInput, setUserInput] = useState(data.config?.userInput || "");
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AIChatbotNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AIChatbotNode ${id}: Chat response - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
      setError(null);
  ...
  const handleDelete = () => {
    ...
    log.info(`AIChatbotNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `AIChatbotNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `AIChatbotNode ${id}: Configuration saved - ${description}, Input: ${userInput}`
    );
  };
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Simulate customer support chat"
        ...
        <div className="flex items-center gap-2">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable AI Chatbot" : "Enable AI Chatbot"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
