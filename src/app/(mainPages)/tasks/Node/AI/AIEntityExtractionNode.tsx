...
const AIEntityExtractionNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [text, setText] = useState(data.config?.text || "");
  const [entityType, setEntityType] = useState(
    data.config?.entityType || "all"
  );
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AIEntityExtractionNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AIEntityExtractionNode ${id}: Entities extracted - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`AIEntityExtractionNode ${id}: Deleted`);
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `AIEntityExtractionNode ${id}: Configuration saved - ${description}, Entity Type: ${entityType}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Extract entities from article"
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Apple announced a new product"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
                      <Label htmlFor="entityType" className="text-gray-300">
                        Entity Type
                      </Label>
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Extract Entities</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>
                  {isEnabled
                    ? "Disable Extract Entities"
                    : "Enable Extract Entities"}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
