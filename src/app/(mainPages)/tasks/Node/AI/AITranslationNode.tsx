...
const AITranslationNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [text, setText] = useState(data.config?.text || "");
  const [sourceLang, setSourceLang] = useState(
    data.config?.sourceLang || "auto"
  );
  const [targetLang, setTargetLang] = useState(data.config?.targetLang || "en");
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AITranslationNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AITranslationNode ${id}: Text translated - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`AITranslationNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `AITranslationNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `AITranslationNode ${id}: Configuration saved - ${description}, Target: ${targetLang}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Translate user feedback"
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Bonjour, comment vas-tu?"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
                      <Label htmlFor="sourceLang" className="text-gray-300">
                        Source Language
                      </Label>
                    ...
                    <div>
                      <Label htmlFor="targetLang" className="text-gray-300">
                        Target Language
                      </Label>
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Translate Text</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>
                  {isEnabled
                    ? "Disable Translate Text"
                    : "Enable Translate Text"}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
