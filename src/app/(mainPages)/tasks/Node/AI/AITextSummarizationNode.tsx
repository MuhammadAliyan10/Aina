...
const AITextSummarizationNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [text, setText] = useState(data.config?.text || "");
  const [maxLength, setMaxLength] = useState(data.config?.maxLength || 200);
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AITextSummarizationNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AITextSummarizationNode ${id}: Text summarized - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`AITextSummarizationNode ${id}: Deleted`);
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `AITextSummarizationNode ${id}: Configuration saved - ${description}, Text: ${text}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Long article text here"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Summarize Text</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>
                  {isEnabled
                    ? "Disable Summarize Text"
                    : "Enable Summarize Text"}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
...
export { AITextSummarizationNode };
