...
const AIDataPredictionNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AIDataPredictionNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AIDataPredictionNode ${id}: Prediction made - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`AIDataPredictionNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `AIDataPredictionNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `AIDataPredictionNode ${id}: Configuration saved - ${description}, Model: ${model}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Predict sales trends"
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder='e.g., [{"sales": 1000, "month": 1}]'
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Predict Data</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>
                  {isEnabled ? "Disable Predict Data" : "Enable Predict Data"}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
