...
const WorkflowNode = ({ id, data }: NodeProps) => {
  ...
  const [workflowId, setWorkflowId] = useState(data.config?.workflowId || ""); // ID of sub-workflow to execute
  const [parameters, setParameters] = useState(data.config?.parameters || ""); // JSON string for parameters
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`WorkflowNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `WorkflowNode ${id}: Executed with output - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleSave = () => {
    ...
    log.info(`WorkflowNode ${id}: Configuration saved - ${description}`);
  };
  ...
  return (
    ...
    >
      ...
      <Handle
        ...
        style={{ width: "0.6rem", height: "0.6rem", background: "#6865A5" }}
      />
      <Handle
        type="source"
