...
const DelayNode = ({ id, data }: NodeProps) => {
  ...
  const [delay, setDelay] = useState(data.config?.delay || 500); // Delay in milliseconds
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`DelayNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(`DelayNode ${id}: Delayed execution for ${delay}ms`);
    } else {
      setStatus("idle");
    }
  }, [data.error, data.output, id, delay]);
  ...
  const handleSave = () => {
    ...
    log.info(
      `DelayNode ${id}: Configuration saved - ${description}, Delay: ${delay}ms`
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start gap-3">
        ...
        <p className="text-xs text-gray-300">{delay} ms</p>
      </div>
