...
const ExportDataNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`ExportDataNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `ExportDataNode ${id}: Data exported - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
    }
  ...
  const handleSave = () => {
    ...
    log.info(
      `ExportDataNode ${id}: Configuration saved - ${description}, Format: ${fileFormat}, Destination: ${destination}, Filename: ${filename}`
    );
  };
  ...
  return (
    ...
    >
      ...
      <Handle
        ...
        style={{ width: "0.6rem", height: "0.6rem", background: "#ff0072" }} // Default color from your scheme
      />
      <Handle
        type="source"
