...
const WaitConnectionNode = ({ id, data }: NodeProps) => {
  ...
  const [wait, setWait] = useState(data.config?.wait || 5000); // Wait duration in ms
  const [retries, setRetries] = useState(data.config?.retries || 3); // Number of retry attempts
  const [timeout, setTimeout] = useState(data.config?.timeout || 30000); // Total timeout in ms
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`WaitConnectionNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `WaitConnectionNode ${id}: Waiting completed - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleSave = () => {
    ...
    log.info(
      `WaitConnectionNode ${id}: Configuration saved - ${description}, Wait: ${wait}ms, Retries: ${retries}, Timeout: ${timeout}ms`
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start gap-3">
        ...
        <p className="text-xs text-gray-300">{wait} ms</p>
      </div>
