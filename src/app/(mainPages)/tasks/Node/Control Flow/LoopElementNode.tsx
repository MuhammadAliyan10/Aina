...
const LoopElementNode = ({ id, data }: NodeProps) => {
  ...
  const [variableName, setVariableName] = useState(
    data.config?.variableName || "element"
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`LoopElementNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `LoopElementNode ${id}: Elements looped - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`LoopElementNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `LoopElementNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `LoopElementNode ${id}: Configuration saved - ${description}, Selector: ${selectorValue}, Variable: ${variableName}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Loop through all list items"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Loop Elements</span>
          {/* Status Indicator */}
          <span
        ...
        <p className="text-xs text-gray-300 capitalize">
          ...
          {selectorValue || "Not Set"}, Var: {variableName}
        </p>
      </div>
