...
const LoopDataNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [dataSource, setDataSource] = useState(
    data.config?.dataSource || "input"
  ); // "input" or "manual"
  const [manualData, setManualData] = useState(data.config?.manualData || ""); // JSON string for manual data
  const [variableName, setVariableName] = useState(
    data.config?.variableName || "item"
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`LoopDataNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `LoopDataNode ${id}: Data looped - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
    }
  ...
  const handleDelete = () => {
    ...
    log.info(`LoopDataNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(`LoopDataNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`);
  };
  ...
  const validateInputs = () => {
    ...
    if (dataSource === "manual" && !manualData.trim()) {
      setError("Manual data is required when data source is manual");
      return false;
    }
    if (dataSource === "manual") {
      try {
        ...
        if (!Array.isArray(parsedData)) {
          setError("Manual data must be a valid JSON array");
          return false;
        }
        if (parsedData.length === 0) {
          setError("Manual data array cannot be empty");
          return false;
        }
      } catch (e) {
        setError("Manual data must be valid JSON");
        return false;
      }
    }
  ...
  const handleSave = () => {
          ...
          ? {
              ...
              data: {
                ...
                config: {
                  ...node.data.config,
                  dataSource,
                  manualData: dataSource === "manual" ? manualData : undefined,
                  variableName,
                  delay: Number(delay),
    ...
    log.info(
      `LoopDataNode ${id}: Configuration saved - ${description}, Source: ${dataSource}, Variable: ${variableName}`
    );
  };
    ...
    >
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="bg-gray-800 text-white rounded-lg shadow-xl p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {dataSource === "manual" && (
                      <div>
                        <Label htmlFor="manualData" className="text-gray-300">
                          Manual Data (JSON Array)
                        </Label>
                        <Textarea
                          id="manualData"
                          value={manualData}
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Loop Data</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable Loop" : "Enable Loop"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        <p className="text-xs text-gray-300 capitalize">
          Source: {dataSource}, Var: {variableName}
        </p>
      </div>
