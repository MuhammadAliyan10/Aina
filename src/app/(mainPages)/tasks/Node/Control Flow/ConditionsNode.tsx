...
const ConditionsNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`ConditionsNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `ConditionsNode ${id}: Condition evaluated - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`ConditionsNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `ConditionsNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `ConditionsNode ${id}: Configuration saved - ${description}, Type: ${conditionType}, Selector: ${selectorValue}`
    );
  };
        ...
        <div className="flex items-center gap-2">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable Conditions" : "Enable Conditions"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
