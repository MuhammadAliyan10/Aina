...
import { Undo2, Edit, Trash, Power, PowerOff, AlertCircle } from "lucide-react";
...
const GoBackNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [steps, setSteps] = useState(data.config?.steps || 1); // Number of history steps to go back
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`GoBackNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `GoBackNode ${id}: Navigated back - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
    }
  ...
  const handleDelete = () => {
    ...
    log.info(`GoBackNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(`GoBackNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`);
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `GoBackNode ${id}: Configuration saved - ${description}, Steps: ${steps}, Confirm: ${confirmNavigation}`
    );
  };
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Navigate back one step"
        ...
        <div className="flex items-center gap-2">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable Go Back" : "Enable Go Back"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
