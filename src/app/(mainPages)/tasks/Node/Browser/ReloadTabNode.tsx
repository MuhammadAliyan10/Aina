...
const ReloadTabNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [reloadType, setReloadType] = useState(
    data.config?.reloadType || "normal"
  ); // normal or hard reload
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`ReloadTabNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `ReloadTabNode ${id}: Tab reloaded - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
    }
  ...
  const handleDelete = () => {
    ...
    log.info(`ReloadTabNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `ReloadTabNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `ReloadTabNode ${id}: Configuration saved - ${description}, Type: ${reloadType}, Timeout: ${timeout}ms`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Reload current tab"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
                      <Label htmlFor="reloadType" className="text-gray-300">
                        Reload Type
                      </Label>
                      <Select value={reloadType} onValueChange={setReloadType}>
                        <SelectTrigger
                          id="reloadType"
                          className="bg-gray-700 border-none text-white"
                        >
                          <SelectValue placeholder="Select reload type" />
                        </SelectTrigger>
                        <SelectContent className="bg-gray-700 text-white">
                          <SelectItem value="normal">Normal</SelectItem>
                          <SelectItem value="hard">Hard Reload</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <div>
                    ...
                    <div>
                      <Label htmlFor="confirmReload" className="text-gray-300">
                        Confirm Reload
                      </Label>
                      <Select
                        value={confirmReload}
                        onValueChange={setConfirmReload}
                      >
                        <SelectTrigger
                          id="confirmReload"
                          className="bg-gray-700 border-none text-white"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Reload Tab</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable Reload Tab" : "Enable Reload Tab"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        <p className="text-xs text-gray-300 capitalize">{reloadType} Reload</p>
      </div>
