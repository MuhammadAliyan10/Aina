...
const BrowserEventNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [eventType, setEventType] = useState(
    data.config?.eventType || "tabLoaded"
  ); // Renamed method to eventType
  const [timeout, setTimeout] = useState(data.config?.timeout || 5000); // Timeout in milliseconds
  const [urlPattern, setUrlPattern] = useState(data.config?.urlPattern || ""); // Optional URL filter
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`BrowserEventNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `BrowserEventNode ${id}: Event ${eventType} detected - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  }, [data.error, data.output, id, eventType]);

  const handleDelete = () => {
    setNodes((nodes) => nodes.filter((node) => node.id !== id));
    log.info(`BrowserEventNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `BrowserEventNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const validateInputs = () => {
    ...
    if (urlPattern && !isValidUrlPattern(urlPattern)) {
  ...
  const handleSave = () => {
    ...
    log.info(
      `BrowserEventNode ${id}: Configuration saved - ${description}, Event: ${eventType}, Timeout: ${timeout}ms`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Wait for tab to load"
                    ...
                    <div>
                      ...
                      <Select value={eventType} onValueChange={setEventType}>
                        ...
                        <SelectContent className="bg-gray-700 text-white">
                          <SelectItem value="tabLoaded">Tab Loaded</SelectItem>
                          <SelectItem value="tabCreated">
                            Tab Created
                          </SelectItem>
                          <SelectItem value="tabClosed">Tab Closed</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <div>
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Browser Event</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>
                  {isEnabled ? "Disable Browser Event" : "Enable Browser Event"}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
