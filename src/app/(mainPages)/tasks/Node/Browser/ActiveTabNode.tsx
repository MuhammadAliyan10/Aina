...
const ActiveTabNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [action, setAction] = useState(data.config?.action || "get-url"); // e.g., get-url, get-title, focus
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`ActiveTabNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `ActiveTabNode ${id}: Action ${action} - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
    }
  }, [data.error, data.output, id, action]);

  const handleDelete = () => {
    setNodes((nodes) => nodes.filter((node) => node.id !== id));
    log.info(`ActiveTabNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `ActiveTabNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `ActiveTabNode ${id}: Configuration saved - ${description}, Action: ${action}`
    );
  };
    ...
    >
                    ...
                    <div>
                      ...
                      <Select value={action} onValueChange={setAction}>
                        ...
                        <SelectContent className="bg-gray-700 text-white">
                          <SelectItem value="get-url">Get URL</SelectItem>
                          <SelectItem value="get-title">Get Title</SelectItem>
                          <SelectItem value="focus">Focus Tab</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    {error && (
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Active Tab</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable Active Tab" : "Enable Active Tab"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
      ...
      {/* Handle (Source only, as it’s a getter node) */}
      <Handle
        type="source"
