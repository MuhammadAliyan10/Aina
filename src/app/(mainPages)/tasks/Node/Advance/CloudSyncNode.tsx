...
import { Cloud, Edit, Trash, Power, PowerOff, AlertCircle } from "lucide-react";
...
const CloudSyncNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [service, setService] = useState(data.config?.service || "aws");
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`CloudSyncNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(`CloudSyncNode ${id}: Synced - ${JSON.stringify(data.output)}`);
    } else {
      setStatus("idle");
      setError(null);
    }
  ...
  const handleDelete = () => {
    ...
    log.info(`CloudSyncNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `CloudSyncNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
        ...
        node.id === id
          ? {
              ...
              data: {
                ...node.data,
                description,
                config: { ...node.data.config, service, isEnabled },
              },
            }
          : node
      )
    ...
    log.info(
      `CloudSyncNode ${id}: Configuration saved - ${description}, Service: ${service}`
    );
  };
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Sync logs to AWS"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Cloud Sync</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {service && (
          <div className="text-xs text-gray-300 flex items-center gap-1">
            <Cloud size={12} />
            <span className="truncate max-w-[9rem]">{service}</span>
          </div>
        )}
      </div>
