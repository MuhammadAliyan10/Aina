...
import { Bug, Edit, Trash, Power, PowerOff, AlertCircle } from "lucide-react";
...
const DebugBreakpointNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`DebugBreakpointNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `DebugBreakpointNode ${id}: Paused - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
      setError(null);
  ...
  const handleDelete = () => {
    ...
    log.info(`DebugBreakpointNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `DebugBreakpointNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
        ...
        node.id === id
          ? {
              ...
              data: {
                ...node.data,
                description,
                config: { ...node.data.config, isEnabled },
              },
            }
          : node
      )
    ...
    log.info(`DebugBreakpointNode ${id}: Configuration saved - ${description}`);
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Pause here for inspection"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Debug Breakpoint</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Debug Breakpoint</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
