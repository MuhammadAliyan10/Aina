...
const ScrollElementNode = ({ id, data }: NodeProps) => {
  ...
  const [scrollBehavior, setScrollBehavior] = useState(
    data.config?.scrollBehavior || "smooth"
  ); // smooth or auto
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`ScrollToElementNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `ScrollToElementNode ${id}: Scrolled to element - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`ScrollToElementNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `ScrollToElementNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `ScrollToElementNode ${id}: Configuration saved - ${description}, Selector: ${selectorValue}, Behavior: ${scrollBehavior}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Scroll to footer"
                    ...
                    <div>
                      <Label htmlFor="scrollBehavior" className="text-gray-300">
                        Scroll Behavior
                      </Label>
                      <Select
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Scroll to Element</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable Scroll" : "Enable Scroll"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
