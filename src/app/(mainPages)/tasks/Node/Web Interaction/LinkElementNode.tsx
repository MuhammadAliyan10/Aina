...
const LinkEventNode = ({ id, data }: NodeProps) => {
  ...
  const [waitForNavigation, setWaitForNavigation] = useState(
    data.config?.waitForNavigation || "load"
  ); // Navigation wait option
  const [retryOnFail, setRetryOnFail] = useState(
    data.config?.retryOnFail || false
  ); // Retry if navigation fails
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`FollowLinkNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `FollowLinkNode ${id}: Link followed - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
    }
  ...
  const handleDelete = () => {
    ...
    log.info(`FollowLinkNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `FollowLinkNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `FollowLinkNode ${id}: Configuration saved - ${description}, Selector: ${selectorValue}, Wait: ${waitForNavigation}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Follow login link"
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={setWaitForNavigation}
                      >
                        <SelectTrigger
                          id="waitForNavigation"
                          className="bg-gray-700 border-none text-white"
                        >
                          <SelectValue placeholder="Select navigation wait" />
                        </SelectTrigger>
                        <SelectContent className="bg-gray-700 text-white">
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Follow Link</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>
                  {isEnabled ? "Disable Follow Link" : "Enable Follow Link"}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
