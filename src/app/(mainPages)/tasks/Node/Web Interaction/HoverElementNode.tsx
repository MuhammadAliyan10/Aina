...
const HoverElementNode = ({ id, data }: NodeProps) => {
  ...
  const [hoverDuration, setHoverDuration] = useState(
    data.config?.hoverDuration || 1000
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`HoverElementNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `HoverElementNode ${id}: Element hovered - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`HoverElementNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `HoverElementNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const validateInputs = () => {
    ...
    const hoverDurationNum = Number(hoverDuration);
    if (isNaN(hoverDurationNum) || hoverDurationNum < 0) {
      setError("Hover duration must be a non-negative number");
      return false;
    }
  ...
  const handleSave = () => {
          ...
          ? {
              ...
              data: {
                ...
                config: {
                  ...
                  selectorType,
                  selectorValue,
                  hoverDuration: Number(hoverDuration),
    ...
    log.info(
      `HoverElementNode ${id}: Configuration saved - ${description}, Selector: ${selectorValue}, Duration: ${hoverDuration}ms`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Hover over menu item"
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={
                          selectorType === "css"
                            ? "e.g., #menu-item"
                            : "e.g., //div[@id='menu-item']"
                        }
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
                      <Label htmlFor="hoverDuration" className="text-gray-300">
                        Hover Duration (ms)
                      </Label>
                      <Input
                        id="hoverDuration"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Hover Element</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable Hover" : "Enable Hover"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
