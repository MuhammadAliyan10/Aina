...
const GetTextNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`GetTextNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `GetTextNode ${id}: Text retrieved - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
    }
  ...
  const handleDelete = () => {
    ...
    log.info(`GetTextNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(`GetTextNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`);
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `GetTextNode ${id}: Configuration saved - ${description}, Selector: ${selectorValue}, Variable: ${
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Get text from header"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Get Text</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable Get Text" : "Enable Get Text"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
