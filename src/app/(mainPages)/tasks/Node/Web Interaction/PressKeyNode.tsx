...
const PressKeyNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [targetElement, setTargetElement] = useState(
    data.config?.targetElement || "global"
  ); // global or specific element
  const [selectorType, setSelectorType] = useState(
    data.config?.selectorType || "css"
  ); // css or xpath (if specific element)
  const [selectorValue, setSelectorValue] = useState(
    data.config?.selectorValue || ""
  ); // Element selector (if specific)
  const [key, setKey] = useState(data.config?.key || "Enter"); // Key to press
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`PressKeyNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `PressKeyNode ${id}: Key pressed - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
    }
  ...
  const handleDelete = () => {
    ...
    log.info(`PressKeyNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(`PressKeyNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`);
  };
  ...
  const validateInputs = () => {
    ...
    if (targetElement === "specific" && !selectorValue.trim()) {
      setError("Selector value is required for specific element");
      return false;
    }
    if (!key.trim()) {
  ...
  const handleSave = () => {
          ...
          ? {
              ...
              data: {
                ...
                config: {
                  ...
                  targetElement,
                  selectorType:
                    targetElement === "specific" ? selectorType : undefined,
                  selectorValue:
                    targetElement === "specific" ? selectorValue : undefined,
                  key,
    ...
    log.info(
      `PressKeyNode ${id}: Configuration saved - ${description}, Key: ${key}, Target: ${targetElement}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Press Enter on input"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
                      <Label htmlFor="targetElement" className="text-gray-300">
                        Target
                      </Label>
                      <Select
                        value={targetElement}
                        onValueChange={setTargetElement}
                      >
                        <SelectTrigger
                          id="targetElement"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Press Key</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable Press Key" : "Enable Press Key"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
