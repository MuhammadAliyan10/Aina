...
import { Mouse, Edit, Trash, Power, PowerOff, AlertCircle } from "lucide-react";
...
const ClickElementNode = ({ id, data }: NodeProps) => {
  ...
  const [clickButton, setClickButton] = useState(
    data.config?.clickButton || "left"
  ); // left, right, middle
  const [clickCount, setClickCount] = useState(data.config?.clickCount || 1); // Number of clicks
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`ClickElementNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `ClickElementNode ${id}: Element clicked - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`ClickElementNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `ClickElementNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const validateInputs = () => {
    ...
    const countNum = Number(clickCount);
    if (isNaN(countNum) || countNum < 1) {
      setError("Click count must be a positive number");
      return false;
    }
    setError(null);
    return true;
  ...
  const handleSave = () => {
    ...
    log.info(
      `ClickElementNode ${id}: Configuration saved - ${description}, Selector: ${selectorValue}, Button: ${clickButton}`
    );
  };
    ...
    >
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="bg-gray-800 text-white rounded-lg shadow-xl p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <Label htmlFor="clickButton" className="text-gray-300">
                        Click Button
                      </Label>
                      <Select
                        value={clickButton}
                        onValueChange={setClickButton}
                      >
                        <SelectTrigger
                          id="clickButton"
                          className="bg-gray-700 border-none text-white"
                    ...
                    <div>
                      <Label htmlFor="clickCount" className="text-gray-300">
                        Click Count
                      </Label>
                      <Input
                        id="clickCount"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Click Element</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable Click" : "Enable Click"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
