...
const GetAttributeNode = ({ id, data }: NodeProps) => {
  ...
  const [attributeName, setAttributeName] = useState(
    data.config?.attributeName || "href"
  ); // Attribute to retrieve
  const [variableName, setVariableName] = useState(
    data.config?.variableName || "attributeValue"
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`GetAttributeNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `GetAttributeNode ${id}: Attribute retrieved - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`GetAttributeNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `GetAttributeNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `GetAttributeNode ${id}: Configuration saved - ${description}, Selector: ${selectorValue}, Attribute: ${attributeName}, Variable: ${variableName}`
    );
  };
    ...
    >
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="bg-gray-800 text-white rounded-lg shadow-xl p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <Label htmlFor="attributeName" className="text-gray-300">
                        Attribute Name
                      </Label>
                      <Input
                        id="attributeName"
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., attributeValue"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Get Attribute</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>
                  {isEnabled ? "Disable Get Attribute" : "Enable Get Attribute"}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        ...
        <p className="text-xs text-gray-300 capitalize">
          ...
          {selectorValue || "Not Set"} - {attributeName}
        </p>
      </div>
