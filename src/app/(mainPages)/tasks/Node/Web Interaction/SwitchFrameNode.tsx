...
const SwitchFrameNode = ({ id, data }: NodeProps) => {
  ...
  const [frameMethod, setFrameMethod] = useState(
    data.config?.frameMethod || "selector"
  ); // selector, index, or parent
  const [selectorType, setSelectorType] = useState(
    data.config?.selectorType || "css"
  ); // css or xpath (for selector method)
  const [selectorValue, setSelectorValue] = useState(
    data.config?.selectorValue || ""
  ); // Frame selector (for selector method)
  const [frameIndex, setFrameIndex] = useState(data.config?.frameIndex || 0); // Frame index (for index method)
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`SwitchFrameNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `SwitchFrameNode ${id}: Frame switched - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
    }
  ...
  const handleDelete = () => {
    ...
    log.info(`SwitchFrameNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `SwitchFrameNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const validateInputs = () => {
    ...
    if (frameMethod === "selector" && !selectorValue.trim()) {
      setError("Selector value is required for selector method");
      return false;
    }
    if (frameMethod === "index") {
      const indexNum = Number(frameIndex);
      if (isNaN(indexNum) || indexNum < 0) {
        setError("Frame index must be a non-negative number");
        return false;
      }
    }
  ...
  const handleSave = () => {
          ...
          ? {
              ...
              data: {
                ...
                config: {
                  ...
                  frameMethod,
                  selectorType:
                    frameMethod === "selector" ? selectorType : undefined,
                  selectorValue:
                    frameMethod === "selector" ? selectorValue : undefined,
                  frameIndex:
                    frameMethod === "index" ? Number(frameIndex) : undefined,
    ...
    log.info(
      `SwitchFrameNode ${id}: Configuration saved - ${description}, Method: ${frameMethod}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Switch to iframe"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
                      <Label htmlFor="frameMethod" className="text-gray-300">
                        Frame Selection Method
                      </Label>
                      <Select
                        value={frameMethod}
                        onValueChange={setFrameMethod}
                      >
                        <SelectTrigger
                          id="frameMethod"
                          className="bg-gray-700 border-none text-white"
                        >
                          <SelectValue placeholder="Select frame method" />
                        </SelectTrigger>
                        <SelectContent className="bg-gray-700 text-white">
                          <SelectItem value="selector">Selector</SelectItem>
                          <SelectItem value="index">Index</SelectItem>
                          <SelectItem value="parent">Parent Frame</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    {frameMethod === "selector" && (
                      <>
                        <div>
                          <Label
                    ...
                    {frameMethod === "index" && (
                      <div>
                        <Label htmlFor="frameIndex" className="text-gray-300">
                          Frame Index
                        </Label>
                        <Input
                          id="frameIndex"
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Switch Frame</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>
                  {isEnabled ? "Disable Switch Frame" : "Enable Switch Frame"}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
