...
const TriggerEventNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`TriggerEventNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `TriggerEventNode ${id}: Event triggered - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`TriggerEventNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `TriggerEventNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const validateInputs = () => {
    ...
    if (eventProperties) {
      try {
        ...
        setError("Event properties must be valid JSON");
        return false;
      }
    }
    setError(null);
  ...
  const handleSave = () => {
    ...
    log.info(
      `TriggerEventNode ${id}: Configuration saved - ${description}, Selector: ${selectorValue}, Event: ${eventType}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Trigger click on button"
                    ...
                    <div>
                      ...
                      <Select value={eventType} onValueChange={setEventType}>
                        ...
                        <SelectContent className="bg-gray-700 text-white">
                          ...
                          <SelectItem value="keydown">Keydown</SelectItem>
                    ...
                    <div>
                      <Label
                        htmlFor="eventProperties"
                        className="text-gray-300"
                      >
                        Event Properties (JSON, Optional)
                      </Label>
                      <Textarea
                        id="eventProperties"
                        value={eventProperties}
                        onChange={(e) => setEventProperties(e.target.value)}
                        placeholder='e.g., {"bubbles": true, "key": "Enter"}'
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Trigger Event</span>
          {/* Status Indicator */}
          <span
        ...
        <p className="text-xs text-gray-300 capitalize">
          ...
          {selectorValue || "Not Set"} - {eventType}
        </p>
      </div>
