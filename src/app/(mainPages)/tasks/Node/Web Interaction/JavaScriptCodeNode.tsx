...
const JavaScriptCodeNode = ({ id, data }: NodeProps) => {
  ...
  const [code, setCode] = useState(data.config?.code || ""); // JavaScript code to execute
  const [variableName, setVariableName] = useState(
    data.config?.variableName || ""
  ); // Optional variable for output
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      log.error(`JavaScriptCodeNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `JavaScriptCodeNode ${id}: Code executed - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`JavaScriptCodeNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `JavaScriptCodeNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const validateInputs = () => {
    ...
    // Optionally validate variableName if provided
    if (variableName && !variableName.trim()) {
      setError("Variable name cannot be empty if provided");
      return false;
    }
    setError(null);
    return true;
  ...
  const handleSave = () => {
    ...
    log.info(
      `JavaScriptCodeNode ${id}: Configuration saved - ${description}, Variable: ${
        variableName || "None"
      }`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Execute custom script"
                    ...
                    <div>
                      ...
                      <Textarea
                        ...
                        placeholder="e.g., return document.title;"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500 h-32 font-mono"
                      />
                    </div>
                    <div>
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">JavaScript Code</span>
          {/* Status Indicator */}
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable JavaScript" : "Enable JavaScript"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        <p className="text-xs text-gray-300 truncate max-w-[10rem]">
          {code
            ? `${code.substring(0, 20)}${code.length > 20 ? "..." : ""}`
            : "No Code"}
        </p>
      </div>
