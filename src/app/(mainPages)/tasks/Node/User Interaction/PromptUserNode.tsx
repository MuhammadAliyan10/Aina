...
const PromptUserNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [promptMessage, setPromptMessage] = useState(
    data.config?.promptMessage || ""
  );
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`PromptUserNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `PromptUserNode ${id}: User input received - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`PromptUserNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `PromptUserNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const validateInputs = () => {
    ...
    if (!promptMessage.trim()) {
      setError("Prompt message is required");
      return false;
    }
    setError(null);
    return true;
  ...
  const handleSave = () => {
        ...
        node.id === id
          ? {
              ...
              data: {
                ...node.data,
                description,
                config: { ...node.data.config, promptMessage, isEnabled },
              },
            }
          : node
      )
    ...
    log.info(
      `PromptUserNode ${id}: Configuration saved - ${description}, Message: ${promptMessage}`
    );
  };
    ...
    >
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Prompt User Input</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Prompt User Input</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {promptMessage && (
          <div className="text-xs text-gray-300 flex items-center gap-1">
            ...
            <span className="truncate max-w-[9rem]">{promptMessage}</span>
          </div>
        )}
      </div>
