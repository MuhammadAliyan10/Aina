...
const UserProfileNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [source, setSource] = useState(data.config?.source || "browser");
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`UserProfileNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `UserProfileNode ${id}: Profile retrieved - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`UserProfileNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `UserProfileNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
        ...
        node.id === id
          ? {
              ...
              data: {
                ...node.data,
                description,
                config: { ...node.data.config, source, isEnabled },
              },
            }
          : node
      )
    ...
    log.info(
      `UserProfileNode ${id}: Configuration saved - ${description}, Source: ${source}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Fetch user details"
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="bg-gray-700 border-none text-white p-2 rounded-md w-full focus:ring-2 focus:ring-blue-500"
                      >
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Get User Profile</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Get User Profile</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {source && (
          <div className="text-xs text-gray-300 flex items-center gap-1">
            <User size={12} />
            <span className="truncate max-w-[9rem]">{source}</span>
          </div>
        )}
      </div>
