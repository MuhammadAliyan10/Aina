...
const ConfirmDialogNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [confirmMessage, setConfirmMessage] = useState(
    data.config?.confirmMessage || ""
  );
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`ConfirmDialogNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `ConfirmDialogNode ${id}: User response - ${JSON.stringify(
          data.output
        )}`
      );
    } else {
  ...
  const handleDelete = () => {
    ...
    log.info(`ConfirmDialogNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `ConfirmDialogNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
        ...
        node.id === id
          ? {
              ...
              data: {
                ...node.data,
                description,
                config: { ...node.data.config, confirmMessage, isEnabled },
              },
            }
          : node
      )
    ...
    log.info(
      `ConfirmDialogNode ${id}: Configuration saved - ${description}, Message: ${confirmMessage}`
    );
  };
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="bg-gray-800 text-white rounded-lg shadow-xl p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <Label htmlFor="confirmMessage" className="text-gray-300">
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Confirm Dialog</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {confirmMessage && (
          <div className="text-xs text-gray-300 flex items-center gap-1">
            <CheckCheck size={12} />
            <span className="truncate max-w-[9rem]">{confirmMessage}</span>
          </div>
        )}
      </div>
