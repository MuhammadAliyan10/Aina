...
const AlertUserNode = ({ id, data }: NodeProps) => {
  const [description, setDescription] = useState(data.description || "");
  const [alertMessage, setAlertMessage] = useState(
    data.config?.alertMessage || ""
  );
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`AlertUserNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `AlertUserNode ${id}: Alert displayed - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
      setError(null);
  ...
  const handleDelete = () => {
    ...
    log.info(`AlertUserNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `AlertUserNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
        ...
        node.id === id
          ? {
              ...
              data: {
                ...node.data,
                description,
                config: { ...node.data.config, alertMessage, isEnabled },
              },
            }
          : node
      )
    ...
    log.info(
      `AlertUserNode ${id}: Configuration saved - ${description}, Message: ${alertMessage}`
    );
  };
    ...
    >
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Operation completed"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    {error && (
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Alert User</span>
          <span
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Alert User</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {alertMessage && (
          <div className="text-xs text-gray-300 flex items-center gap-1">
            <AlertCircle size={12} />
            <span className="truncate max-w-[9rem]">{alertMessage}</span>
          </div>
        )}
      </div>
