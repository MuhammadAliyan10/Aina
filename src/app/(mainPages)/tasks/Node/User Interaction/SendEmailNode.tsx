...
import { Mail, Edit, Trash, Power, PowerOff, AlertCircle } from "lucide-react";
...
const SendEmailNode = ({ id, data }: NodeProps) => {
  ...
  useEffect(() => {
    if (data.error) {
      setStatus("error");
      setError(data.error);
      log.error(`SendEmailNode ${id}: ${data.error}`);
    } else if (data.output) {
      setStatus("running");
      log.info(
        `SendEmailNode ${id}: Email sent - ${JSON.stringify(data.output)}`
      );
    } else {
      setStatus("idle");
      setError(null);
  ...
  const handleDelete = () => {
    ...
    log.info(`SendEmailNode ${id}: Deleted`);
  };
  ...
  const handleToggleEnable = () => {
    ...
    log.info(
      `SendEmailNode ${id}: ${newEnabledState ? "Enabled" : "Disabled"}`
    );
  };
  ...
  const handleSave = () => {
    ...
    log.info(
      `SendEmailNode ${id}: Configuration saved - ${description}, To: ${recipient}, Subject: ${subject}`
    );
  };
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Send report to admin"
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Automation Report"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., Here is your report"
                        className="bg-gray-700 border-none text-white p-2 rounded-md focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    {error && (
      ...
      <div className="flex flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="bg-gray-700 text-white">
                <p>{isEnabled ? "Disable" : "Enable"} Send Email</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        {recipient && (
          <div className="text-xs text-gray-300 flex items-center gap-1">
            <Mail size={12} />
            <span className="truncate max-w-[9rem]">{recipient}</span>
          </div>
        )}
      </div>
